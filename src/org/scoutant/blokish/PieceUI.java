...
public class PieceUI extends FrameLayout implements OnTouchListener,
		OnLongClickListener, Comparable<PieceUI> {
	...
	private void doLayout() {
		...
		if (j > 20) {
			layout = new FrameLayout.LayoutParams(df * size, df * size,
					Gravity.TOP);
			layout.leftMargin = (i - 1) * size;
			if (!moving)
				layout.leftMargin -= swipeX * size;
			layout.topMargin = (j - 1) * size;
		} else {
			layout = new FrameLayout.LayoutParams(2 * radius, 2 * radius,
					Gravity.TOP);
	...
	/**
	 * Caution : must invoke doLayout() before any invalidate() if i or j
	 * happened to be updated! As onDraw wont be called if piece is (was) out of
	 * viewport.
	 */
	@Override
	...
	private PieceUI add(int i, int j) {
		...
		if (game.lasts[piece.color] == this && this.j <= 20) {
			square_bold.setBounds(new Rect((i + PADDING + oo) * size, (j
					+ PADDING + oo)
					* size, (i + PADDING + oo + 1) * size + 1, (j + PADDING
					+ oo + 1)
					* size + 1));
		...
		if (this.j <= 20) {
			square.setBounds(new Rect((i + PADDING + oo) * size + 1, (j
					+ PADDING + oo)
					* size + 1, (i + PADDING + oo + 1) * size, (j + PADDING
					+ oo + 1)
					* size));
		} else {
			square.setBounds(new Rect((i + oo) * size + 1, (j + oo) * size + 1,
					(i + oo + 1) * size, (j + oo + 1) * size));
		}
		square.draw(canvas);
		return this;
	}
	...
	public boolean onTouch(View v, MotionEvent event) {
		// TODO possible to hook it in lifecycle? onAttachedToWindow() is to
		// early...
		if (statusBarHeight < 0) {
			Rect decor = new Rect();
			((Activity) context).getWindow().getDecorView()
					.getWindowVisibleDisplayFrame(decor);
		...
		if (game.selected == null
				&& !PreferenceManager.getDefaultSharedPreferences(context)
						.getBoolean("ai", true) && piece.color != game.ui.turn) {
			game.doTouch(event);
			return false;
		}
		...
		if (action == MotionEvent.ACTION_DOWN) {
			...
			if (willRotate()) {
				rotating = true;
				rDown = Math.toDegrees(Math.atan2(event.getX() - radius, radius
						- event.getY()));
			} else {
				rotating = false;
			}
		...
		if (action == MotionEvent.ACTION_MOVE && game.selected == this) {
			bringToFront();
			if (rotating) {
				double r = Math.toDegrees(Math.atan2(event.getX() - radius,
						radius - event.getY()));
				...
				int newj = ((int) event.getRawY() - statusBarHeight - localY + r)
						/ size;
				if (i == newi && j == newj)
					return false;
				i = newi;
		...
		if (action == MotionEvent.ACTION_UP) {
			...
			if (j > 20) {
				...
				boolean okState = game.game.valid(piece, i, j)
						&& !game.thinking;
				game.buttons.setOkState(okState);
				if (okState && vibrator != null)
					vibrator.vibrate(20);
			}
		}
		invalidate();
	...
	public int compareTo(PieceUI that) {
		return (2 * this.piece.count + this.piece.size)
				- (2 * that.piece.count + that.piece.size);
	}
}
