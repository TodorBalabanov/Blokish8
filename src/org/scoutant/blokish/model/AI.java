...
public class AI {
				...
				for (int f = 0; f < piece.flips; f++, piece.flip()) {
					for (int r = 0; r < piece.rotations; r++, piece.rotate(1)) {
						for (Square s : piece.squares()) {
							int i = seed.i - s.i;
							int j = seed.j - s.j;
							if (!board.outside(s, i, j)
									&& game.fits(piece, i, j)) {
								Log.d(tag, "possible move : "
										+ new Move(piece, i, j));
	...
	protected List<Move> thinkUpToNMoves(int color, int level) {
		...
		for (Square seed : seeds) {
			...
			for (int p = 0; p < board.pieces.size()
					&& movesAgainstSeed < maxMovesAgainstSeed; p++) {
				Piece piece = board.pieces.get(p);
				for (int r = 0; r < piece.rotations; r++, piece.rotate(1)) {
					for (int f = 0; f < piece.flips; f++, piece.flip()) {
						for (Square s : piece.squares()) {
							...
							if (!board.outside(s, i, j)
								...
								int seedsIfAdding = board.scoreSeedsIfAdding(
										piece, i, j);
